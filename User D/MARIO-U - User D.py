...
#%% creating csv file
path_csv = "Results/run_cases.csv"
files = [f for f in listdir("Results") if isfile(join("Results", f))]

if path_csv.split("/")[-1] not in files:
    run_cases = pd.DataFrame(columns=["User","Location","Yearly mileage","qi","Electricity price","BEV incentives","Gasoline Price","BEV capacity"])
    run_cases.index.names = ["Case number"]
    run_cases.to_csv(path_csv)
                    ...
                    for bev_incentive in params['BEV incentives']['Values']:
                        ...
                        for gas_price in params['Gasoline Price']['Values']:
                            ...
                            for bev_capacity in params['BEV capacity']['Values']:
                                ...
                                run_cases = pd.read_csv(path_csv, sep=",")
                                if counter not in run_cases.index:
                                    ...
                                    new_case = pd.DataFrame(index=[counter], columns=run_cases.columns)
                                    new_case.loc[counter,"User"] = user
                                    new_case.loc[counter,"Location"] = location
                                    new_case.loc[counter,"Yearly mileage"] = mileage
                                    new_case.loc[counter,"qi"] = year                                
                                    new_case.loc[counter,"Electricity price"] = bev_incentive
                                    new_case.loc[counter,"BEV incentives"] = ee_price
                                    new_case.loc[counter,"Gasoline Price"] = gas_price
                                    new_case.loc[counter,"BEV capacity"] = bev_capacity
                                    
                                    run_cases = pd.concat([run_cases, new_case],axis=0)
                                    run_cases.to_csv(path_csv)
                                    #######
                                
                                else:
                                    print(f"Case {counter} skipped because already present")
