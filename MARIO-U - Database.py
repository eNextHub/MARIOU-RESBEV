                    ...
                    for bev_incentive in params['BEV incentives']['Values']:
                        ...
                        for gas_price in params['Gasoline Price']['Values']:
                            ...
                            for bev_capacity in params['BEV capacity']['Values']:
                                ...
                                #k.loc["Coat","Costo di installazione"] = 2*70*2.7*(house_surface/4 +4)#attivare solo per il caso senza incentivi
...
# for i in range(len(List_imput)):
    
#     pj_fld = List_imput[i]


    # Data Entry
    # input_nt = pd.read_excel(support_path, header=[0,1], sheet_name='n-t')
    # input_f = pd.read_excel(support_path, header=[0,1], sheet_name='f')
    # input_l = pd.read_excel(support_path, header=[0,1], sheet_name='l')
    # input_i = pd.read_excel(support_path, header=[0,1], sheet_name='i')
    # input_time_operation = pd.read_excel(support_path, header=None, index_col=[0], sheet_name='qo')
    # input_time_investment = pd.read_excel(support_path, header=None, index_col=[0], sheet_name='qi')
   ...
   #a = a.append({'Name': 'Charging {} from external'.format(input_nt.loc[ii,(Nt,'Name')]),
                          #'Unit':'{}'.format(input_nt.loc[ii,(Nt,'Type')])},ignore_index=True).drop_duplicates()  
    ...
    # u = pd.DataFrame(0, index=nU_index, columns=aU_index).to_excel('u.xlsx') # Writing indexing with explicit units
    ...
    # v = pd.DataFrame(0, index=fU_index, columns=aU_index).to_excel('{}/v.xlsx'.format(pr_fld)) # Writing indexing with explicit units
    ...
    # e = pd.DataFrame(0, index=lU_index, columns=aU_index).to_excel('{}/e.xlsx'.format(pr_fld)) # Writing indexing with explicit units
    ...
    # k = pd.DataFrame(0, index=kU_index, columns=iU_index).to_excel('{}/k.xlsx'.format(pr_fld))
    # # k_ = pd.DataFrame(0, index=kU_index, columns=iqi_index).to_excel('{}/k_.xlsx'.format(pr_fld))
    
    # Y, matrix of final demands
    # Y = pd.DataFrame(0, index=nU_index, columns=qo).to_excel('Y.xlsx')
    
    #Y_coat, matrix of final demands with thermal coat
    # Y_coat = pd.DataFrame(0, index=nU_index, columns=qo).to_excel('Y_coat.xlsx')
    ...
    # A = pd.DataFrame(big, index=atU_index, columns=qo).to_excel('A.xlsx')
    # At = pd.DataFrame(big, index=kU_index, columns=qo).to_excel('At.xlsx')
    
    #%% Reading-back and re-indexing the matrices filled by the user
    # u = pd.DataFrame(pd.read_excel('{}/u.xlsx'.format(pj_fld), index_col=[0,1], header=[2]).values, index=n_index, columns=a_index)
    # A = pd.DataFrame(pd.read_excel('{}/A.xlsx'.format(pj_fld), index_col=[0,1,2], header=[0]).values, index=a_index, columns=qo)
    # At = pd.DataFrame(pd.read_excel('{}/At.xlsx'.format(pj_fld), index_col=[0,1], header=[0]).values, index=t_index, columns=qo)
    # k = pd.DataFrame(pd.read_excel('{}/k.xlsx'.format(pj_fld), index_col=[0,1], header=[2]).values, index=t_index, columns=i_index)
    # # k_ = pd.DataFrame(pd.read_excel('{}/k_.xlsx'.format(pj_fld), index_col=[0,1], header=[2]).values, index=t_index, columns=iqi_index)
    # e = pd.DataFrame(pd.read_excel('{}/e.xlsx'.format(pj_fld), index_col=[0,1], header=[2]).values, index=l_index, columns=a_index)
    # # e_ = pd.DataFrame(pd.read_excel('{}/e_.xlsx'.format(pj_fld), index_col=[0,1], header=[2]).values, index=l_index, columns=aqo_index)
    # v = pd.DataFrame(pd.read_excel('{}/v.xlsx'.format(pj_fld), index_col=[0,1], header=[2]).values, index=f_index, columns=a_index)
    # # v_ = pd.DataFrame(pd.read_excel('{}/v_.xlsx'.format(pj_fld), index_col=[0,1], header=[2]).values, index=f_index, columns=aqo_index)
    # Y = pd.DataFrame(pd.read_excel('{}/Y.xlsx'.format(pj_fld), index_col=[0,1], header=[0]).values, index=n_index, columns=qo)
    # Y_coat = pd.DataFrame(pd.read_excel('{}/Y_coat.xlsx'.format(pj_fld), index_col=[0,1], header=[0]).values, index=n_index, columns=qo)
    ...
    # add_par = pd.read_excel('Data_entry_{}.xlsx'.format(pj_fld), sheet_name='additional parameters', header=[0], index_col=[0,1])
    # dr = add_par.loc['Discount rate','Value'][0]
    # cp = add_par.loc['CO2 price','Value'][0]
    ...
    # PV_limit = list(t.loc[:,'Name']).index('PV')#Position PV
    # Coat_pos = list(t.loc[:,'Name']).index('Coat') #Position of Coat
    # #Column_pos = list(a.loc[:,'Name']).index('Exploiting External Column for Electricity')#Position of column
...
#     fig = make_subplots(rows=len(n_plt), cols=1, subplot_titles=list(n_plt.index))
    
#     for ni in list(n_plt.index):
#         if ni == 'Electricity':
#             for bi in list(SOC.index):
#                 fig.add_trace(go.Scatter(name='State of charge of {}'.format(bi[0]), x=SOC.columns, y=SOC.loc[bi,:].values, line=dict(dash='dash')), col=1, row=list(n_plt.index).index(ni)+1)
#             fig.add_trace(go.Bar(name='Surplus of {} sold'.format(ni), x=Xres.loc['Selling PV surplus'].columns, y=Xres.loc['Selling PV surplus'].values[0]), col=1, row=list(n_plt.index).index(ni)+1)
#             fig.add_trace(go.Scatter(name='Solar availability', x=Xres.loc['Selling PV surplus'].columns, y=Dres.loc[sol.Name[0]].values[0][0]*A.loc['Selling PV surplus'].values[0]), col=1, row=list(n_plt.index).index(ni)+1)
    
#         fig.add_trace(go.Scatter(name='Final demand of {}'.format(ni), x=Y.columns, y=Y.loc[ni,:].values[0]), col=1, row=list(n_plt.index).index(ni)+1)
...
#         fig.add_trace(go.Bar(name='Thermal coat reduction {}'.format(ni), x=Y_coat.columns, y=Y_coat.loc[ni,:].values[0]+Yint.loc[ni,:].values[0]), col=1, row=list(n_plt.index).index(ni)+1)
    
#         for ti in Xres.loc[(slice(None),slice(None),ni),:].index.get_level_values(1).values:
#             fig.add_trace(go.Bar(name=ti, x=Xres.columns, y=Xres.loc[(slice(None),ti,ni),:].values[0]), col=1, row=list(n_plt.index).index(ni)+1)
    
    
#     fig.update_layout(barmode='relative', title='{}. Investimento totale: {}€'.format(str(Choice),(k_cost.T@Dres.values)[0][0]), showlegend=True, font_family='Palatino Linotype', font_size=20)
#     fig.write_html('{}/Result.html'.format(pj_fld))
    
    
#     print(str(Choice),(k_cost.T@Dres.values)[0][0])
    
#     costioperativi=np.dot(v_cost, Xres)
#     costi=np.sum(costioperativi)
#     print(costi)
    
    
#     print(str(Choice),(k_co2.T@Dres.values )[0][0])
    
#     emiconsumi= np.dot(e_co2, Xres)
#     emissioni=np.sum(emiconsumi)
#     print(emissioni)
    
    
#     Dresexcel= pd.ExcelWriter('{}/Dresult.xlsx'.format(pj_fld), engine= 'xlsxwriter')
#     Dres.to_excel(Dresexcel, sheet_name= 'try')
#     Dresexcel.save()
    
#     Xresexcel= pd.ExcelWriter('{}/Xresult.xlsx'.format(pj_fld), engine= 'xlsxwriter')
#     Xres.to_excel(Xresexcel, sheet_name= 'try')
#     Xresexcel.save()
    
    
#     #Yintexcel= pd.ExcelWriter('Yresult.xlsx', engine= 'xlsxwriter')
#     #Yint.to_excel(Yintexcel, sheet_name= 'try')
#     #Yintexcel.save()
    
#     SOCexcel= pd.ExcelWriter('{}/SOCresult.xlsx'.format(pj_fld), engine= 'xlsxwriter')
#     SOC.to_excel(SOCexcel, sheet_name= 'try')
#     SOCexcel.save()
...
#     Choice = Dres.loc[Dres.loc[:,'Installed units']!=0]
#     drop_needs = ['Cooling','Cooking','Hot Sanitary Water','Heating','Thermal coating']
#     n_plt = n.set_index('Name').drop(drop_needs)
    
#     size_plt = [0.7] # first value for Ele
...
#             fig.add_trace(go.Bar(name='Surplus of {} sold'.format(ni), x=Xres.loc['Selling PV surplus'].columns, y=Xres.loc['Selling PV surplus'].values[0], marker=dict(color='gold')), col=1, row=list(n_plt.index).index(ni)+1)
#             fig.add_trace(go.Scatter(name='Solar availability', x=Xres.loc['Selling PV surplus'].columns, y=Dres.loc[sol.Name[0]].values[0][0]*A.loc['Selling PV surplus'].values[0]), col=1, row=list(n_plt.index).index(ni)+1)
